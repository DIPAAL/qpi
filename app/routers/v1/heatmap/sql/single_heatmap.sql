WITH reference (rast, geom) AS (
 ...
 SELECT
    ST_AsGDALRaster(
        ST_MapAlgebra(q2.rast, reference.rast, '[rast1.val]+[rast2.val]', extenttype:='UNION'),
        'GTiff'
    ) AS raster
FROM reference,
     (
         ...
         FROM (
                  ...
                  WHERE fch.spatial_resolution = :spatial_resolution
                    AND fch.heatmap_type_id = (
                                                  SELECT heatmap_type_id
                                                  FROM dim_heatmap_type
                                                  WHERE slug = :heatmap_type_slug
                                              )
